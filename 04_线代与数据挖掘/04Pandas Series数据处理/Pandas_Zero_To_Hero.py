...
"""
loc 包含两端：df.loc[0:1] 会包含索引为 0 和 1 的行
iloc 左闭右开：df.iloc[0:2] 会包含位置为 0 和 1 的行，但不包含位置为 2 的行
标签 vs 位置：如果你的 DataFrame 有自定义索引（如字符串索引），
    loc 使用的是标签，
    而 iloc 始终使用整数位置
这两种索引方式为数据选择提供了灵活性，你可以根据具体情况选择最适合的一种。
"""
