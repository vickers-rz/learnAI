...
class LinkList:
    ...
    def insert(self, pos, data):
        ...
        3. 否则找到指定位置的前一个节点，在其后插入新节点。
            # 将pre节点的next先拿到，给到newNode的next里；
            # 再将pre节点的next改成newNode的内存指针地址。
        ...
        # 如果位置超出范围，在尾部插入
            ...
            while count < (pos - 1):
                count += 1           # 找到要插入位置的前一个节点
                pre = pre.next       # pre = pre.next一直在循环就实现了一个游标。得到前一个节点的next域。
                # 注意“pre = pre.next”语句与累加“count += 1”语句的上下位置关系。
                # 关系到到底是否是pre.next，还是pre.next.next。

            # 修改指针完成插入操作。
            # 也要特别注意以下两句的前后顺序，否则会丢失后一个节点的指针信息。
    ...
    def remove(self, data):
        ...
        实现原理：
        1. 遍历链表找到要删除的节点（也就是下面说的current节点），获取其原本的next区域所存储的原本的下个节点的地址；
        2. 使用两个指针pre（前一个节点）和current（当前节点）；
        3. 将pre节点的next改成current节点的next,以跳过要删除的节点的链。
